...
#   coole stuff from Christian Wartena
...
#
#text = sys.argv[1]
text = '/Users/Fabi/PycharmProjects/Thesis/ANALYSIS/sample/11-Niemand_ist_ein_Nazi_Niemand_ist_je_einer_gewesen-05-04-2020.csv'
...
#        answer = input("Do you want to see the unique items? (y/n)")
        ...
        print("Dataframe converted to list &  Start Emotion Analysis Plutschik ")
        if len(uniq) == 0 :
            sys.exit('Keine Textdaten nach Entfernung Stopwords')
#PD

        for index, row in self.datafr.iterrows():
            self.labels = {} # Note: Hier hatte ich für sehr lange Zeit ein drecks Fehler. Scheinbar darf labels nicht schon in der __init__ stehen
...
#        self.merged_df.sort_values('Häufigkeit')
        ...
        print("---------------------SCHAU MAL HIER OBEN-----------------------------------------")
        self.merged_df.dtypes
        self.merged_df.to_csv(f'mrg_8emotions_{date}.csv',sep=";")
...
#        print(self.E_sum[2],self.E_sum[3])
        ...
        print("                                            ")
        text = colored('     Polarity NRC :', 'green', attrs=['reverse', 'blink'])
        if pol_nrc < 0 :
           text = colored('     Polarity NRC :', 'red', attrs=['reverse', 'blink'])
        lenworig = len(self.clean_data)
        emmo_index1 = p_sum/lenworig
        emmo_index2 = all_emmos/lenworig
        print("                                            ")
        print(text,round(pol_nrc,3))
        print("     Polarity NRC :",round(pol_nrc,3))
        print(" Emotionsindex EI_1 :",round(emmo_index1,3))
        print(" Emotionsindex EI_2 :",round(emmo_index2,3))
# 
